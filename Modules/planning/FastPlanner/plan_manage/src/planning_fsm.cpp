...
void PlanningFSM::init(ros::NodeHandle& nh)
{
  ...
  // [PUB]发布新的目标点
  goalpoint_pub_ = node_.advertise<geometry_msgs::PoseStamped>("/prometheus/planning/goal", 10);
...
void PlanningFSM::waypointCallback(const geometry_msgs::PoseStampedConstPtr& msg)
{
  ...
  pub_message(message_pub, prometheus_msgs::Message::NORMAL,  NODE_NAME, "Get a new goal: [ " + to_string(end_pt_(0)) + "," + to_string(end_pt_(1)) + "," + to_string(end_pt_(2)) + "]!---");
...
void PlanningFSM::execFSMCallback(const ros::TimerEvent& e)
{
  ...
  if (fsm_num == 100)
  {
    printExecState();
    if (!trigger_)
      pub_message(message_pub, prometheus_msgs::Message::NORMAL,  NODE_NAME, "haven't trigger!.\n");
...
void PlanningFSM::safetyCallback(const ros::TimerEvent& e)
{
  ...
  if(cur_safe_dist< safety_distance){
    replan.data = 1; // stop!
    pub_message(message_pub, prometheus_msgs::Message::WARN,  NODE_NAME, "[safetyCallback]: current position dangerous, stop and wait a new goal\n");
    changeExecState(WAIT_GOAL, "FSM");
    std_msgs::Empty emt;
...
bool PlanningFSM::planSearchOpt()
{
  ...
  if (plan_success)
  {
    ...
    /* visulization */
    vector<Eigen::Vector3d> kino_path = path_finder_->getKinoTraj(0.02);// kinodynamic A* path
